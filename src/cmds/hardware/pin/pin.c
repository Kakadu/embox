 ...
 * @param gpio Output parameter. If function returns successfully contains
 *             information about specified GPIO.
 ...
 * @return TRUE if functions successfully parsed GPIO and (maybe) pin index
 *         from input string. On fail returns FALSE.
 */
static int gpio_by_name(char *name, void **gpio, gpio_mask_t *pin) {
	...
	switch (name[0]) {
	case 'a':	case 'A': *gpio = GPIOA; return 1;
	case 'b':	case 'B': *gpio = GPIOB; return 1;
	case 'c':	case 'C': *gpio = GPIOC; return 1;
	case 'd':	case 'D': *gpio = GPIOD; return 1;
	case 'e':	case 'E': *gpio = GPIOE; return 1;
	case 'f':	case 'F': *gpio = GPIOF; return 1;
	}
...
int main(int argc, char **argv) {
	...
	if (strlen(argv[opt]) > 1) {
		// [char]something
		int rez = gpio_by_name(argv[opt], &gpio, &pin);
		if (rez) {
